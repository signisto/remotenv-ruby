describe Envoku::Adapters do
  describe ".self" do
    context "http" do
      let!(:uri) { URI("http://example.com/test.env") }
      subject { Envoku::Adapters.for(uri) }

      it "correctly returns http adapter" do
        expect(subject.is_a?(Envoku::Adapters::Http)).to eq(true)
      end
    ...
    context "s3" do
      ...
      it "correctly returns http adapter" do
        expect(subject.is_a?(Envoku::Adapters::S3)).to eq(true)
      end
